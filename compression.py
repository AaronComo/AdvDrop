...
def block_splitting(image, block_size=8):
    ...
    image_reshaped = desired_img.view(batch_size, desired_height // k, k, -1, k)    # 将image进行reshape成数据一致维度不同的维度，-1表示此维度的大小由其他参数推断出来，变换后为非对齐张量
    image_transposed = image_reshaped.permute(0, 1, 3, 2, 4)
    return image_transposed.contiguous().view(batch_size, -1, k, k)     # contiguous返回一个连续的tensor（元素在内存空间上连续），以便再次进行view变换
...
def dct_8x8(image):
    ...
    # TODO: 没看懂下面的for，上面的tensor为什么是8x8x8x8的
    for x, y, u, v in itertools.product(range(8), repeat=4):    # maps: x:k, y:m, u:i, v:j
...
def quantize(image, q_table, alpha):
    ...
    pre_img = image / q_table    # formula (5) 中 x/q
    after_img = phi_diff(pre_img, alpha)    # 放入 phi_diff()计算 formula (7) (quantize)
